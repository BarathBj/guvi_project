...
#API key connection
def Api_connect():
    Api_Id="AIzaSyDp9yYkdAhGWQ0UIp16LWhc1MNLvgvu42Y"
...
### youtube channel id's here ###
#1.MrVignesh -       UC5GgmQzKX65brzNM9iyys_Q
#2.StudyNow -        UCf_0OLTYkT1BCIYN8vSweBg
#3.PASSIONOFGAMING - UCifJQiqUTDjPvsvFLJ4cPbQ
#4.GUVI            - UCduIoIMfD8tT3KoU0-zBRgQ
#5.Dinavaasal News - UC40K1FY7_BNBsgGDPKQaUAA
#6.India Sports    - UCnNgYiRKrwotiQKkIXSmzVg
#7.Mysterious Story- UCv2RMJGDfkaKeYqQ72zmUyw 
#8.Sound Settai    - UCNfvY8ycvr_ffvPiBWTJp0g
#9.தமிழ்           - UCJZaxLeOi2tAdGNsmgnn-Xw to extract demo
#10.  
###
...
#get commentthreads information
...
#Table creation for channels,playlistsitems, videos, commentsthreads
def channels_table():
...
def tables():
    ...
    videos_table()
    comments_table()
    return "PSQL Tables Created successfully"
...
st.header(":white[Enter YouTube Channel_ID below:]")
st.text("Hint:Go to channel's homepage>>Rightclick>>Viewpage source>>Find channel_id")    
...
# show streamlit mongodb databases
def get_mongo_databases():
    client = MongoClient("mongodb://localhost:27017/")
    return client.list_database_names()

# Function to get MongoDB collection names for a selected database
def get_mongo_collections(Youtube_data):
    client = MongoClient("mongodb://localhost:27017/")
    db = client[Youtube_data]
    return db.list_collection_names()

st.header(":red[SHOW MONGODB]")
st.title("select a channel to begin transformation to SQL")

# Dropdown menu to MongoDB
selected_database = st.selectbox("Select MongoDB Database", get_mongo_databases())

# Button to fetch and display collection names
if st.button("Submit"):
    collections = get_mongo_collections(selected_database)
    st.write("Collections in the selected database:")
    st.write(collections)

    
#PSQL connection for questions and answers
mydb = psycopg2.connect(host="localhost",
            user="postgres",
            password="NewPassword",
            database= "youtube_data",
